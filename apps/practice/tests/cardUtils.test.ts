  ...
  describe('groupCardsBySuit', () => {
    it('groups cards by suit correctly', () => {
      ...
      expect(result).toEqual({
        ...
        time: [{ id: 't1', suit: 'time', title: 'Card 2', description: 'Body 2', type: 'practice' }],
      });
    });
    ...
    it('handles single card', () => {
      const cards: Card[] = [
        { id: 'e1', suit: 'expression', title: 'Solo Card', description: 'Solo Body', type: 'practice' },
      ];

      const result = groupCardsBySuit(cards);

      expect(result).toEqual({
        expression: [{ id: 'e1', suit: 'expression', title: 'Solo Card', description: 'Solo Body', type: 'practice' }],
      });
    });
  });
});
